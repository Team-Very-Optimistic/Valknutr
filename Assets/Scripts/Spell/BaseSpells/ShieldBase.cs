...
class ShieldBase : SpellBase
{
    ...
    {
        return new Tooltip("Shield <Base>", $"Spawns a shield that absorbs {_player.GetComponent<PlayerHealth>().maxHealth/10 + 10 * _scale} damage for the player. Shields also deal {_damage} to enemies that it collides with.");
    }
}